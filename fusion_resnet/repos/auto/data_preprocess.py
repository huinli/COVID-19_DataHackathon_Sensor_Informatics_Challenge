...
if __name__ == "__main__":
    ...
    parser.add_argument("dataset_root", type=Path,
                        help="Path to the directory containing VoxCeleb datasets. It should be arranged as:")
    parser.add_argument("-s", "--skip_existing", action="store_true", help="Whether to skip existing output files with the same name. Useful if this script was "
                        "interrupted.")
    ...
    # if os.path.exists(args.dataset_root.joinpath("feature")):
    ...
    preprocess_voxceleb1(args.dataset_root, 'dev',
                         dev_out_dir, args.skip_existing)
    preprocess_voxceleb1(args.dataset_root, 'test',
                         test_out_dir, args.skip_existing)
    for path in dev_out_dir.iterdir():
        subprocess.call(['cp', '-r', path.as_posix(),
                         merged_out_dir.as_posix()])
    ...
    partition_voxceleb(
        merged_out_dir, args.dataset_root.joinpath('iden_split.txt'))
    print("Done")
