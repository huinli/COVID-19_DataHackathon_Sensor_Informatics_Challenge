...
def main():
    ...
    model = Network(cfg.MODEL.INIT_CHANNELS,
                    cfg.MODEL.NUM_CLASSES, cfg.MODEL.LAYERS, genotype)
    model = model.cuda()
    ...
    if args.load_path and os.path.exists(args.load_path):
        checkpoint_file = os.path.join(
            args.load_path, 'Model', 'checkpoint_best.pth')
    ...
    test_dataset = DeepSpeakerDataset(
        Path(cfg.DATASET.DATA_DIR),  'test', cfg.DATASET.PARTIAL_N_FRAMES,  is_test=True)
    ...
    for epoch in tqdm(range(begin_epoch, cfg.TRAIN.END_EPOCH), desc='train progress'):
        ...
        train_from_scratch(cfg, model, optimizer, train_loader,
